...
Write a `draw(ctx)` method. Draw a circle of the appropriate `radius` centered
...
Write a `Game.prototype.step` method, which calls `moveObjects` then
`checkCollisions`. Call this in `GameView.prototype.start` instead of
