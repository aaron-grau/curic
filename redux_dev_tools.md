...
Redux has it's own special set of developer tools.
They allow you to do things like inspect the state in real time as you use your app, and cancel an action to see a live recalculation of the state as if that action had never been dispatched.
They require only a few minutes of setup, and can be well worth the effort.

The redux dev tools are contained in a custom React component.
In the past one had to add this component to an app in order to use them.
Fortunately, the dev tools are now included in a chrome extension which allows them to open in a separate window.
There are two steps to the setup: 1) install a chrome extension, 2) add custom middleware to your store that allows the dev tools to track the state of your app and log any actions you dispatch.
...
To install the server, run `npm install -g http-server`.
...
You should see a little atom icon on your Chrome toolbar, and if you've setup the dev tools correctly it should now be green.
Click on it.
