...
from the server, so we can take on another `.then` after our promise resolves and
redirect from there.
...
when chaining calls to `then` the return value of the previous as passed as the input
